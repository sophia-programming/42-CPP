...
void	ScalarConverter::display(const std::string &str, int flag) {
	...
	if (((flag >> SHIFT_INT) & MASK_FLAG) == FLAG_MAX)
		std::cout << YELLOW << "int: " << std::numeric_limits<int>::max();
	else if (((flag >> SHIFT_INT) & MASK_FLAG) == FLAG_MIN)
		std::cout << YELLOW << "int: " << std::numeric_limits<int>::min();
	else
		std::cout << "int: " << "impossible" << std::endl;

	std::cout << YELLOW << "float: " << str << "f" << std::endl;
	std::cout << YELLOW << "double: " << str << std::endl;
}
